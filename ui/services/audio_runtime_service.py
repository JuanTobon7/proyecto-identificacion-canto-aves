    ...
    def play_file(
        ...
        def _callback(outdata, frames, time, status) -> None:  # noqa: ARG001
            ...
            if on_position is not None:
                on_position(float(end / sample_rate))

            if end >= total_samples:
                raise sd.CallbackStop()

        with sd.OutputStream(samplerate=sample_rate, channels=1, dtype="float32", callback=_callback):
            if on_position is not None:
                on_position(0.0)
            sd.sleep(int(np.ceil(total_samples / sample_rate * 1000)))
